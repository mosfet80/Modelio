...
public class OidcRefreshTokenFlow implements IOidcAuthenticationFlow {
    ...
    @objid ("d6b68ea3-87e7-4393-8440-8927846ff59b")
    private final Lock getTokenslock;
    ...
    public OidcRefreshTokenFlow(ClientID clientId, Secret clientSecret, ReadOnlyOIDCProviderMetadata metadata, IOidcAuthenticationFlow alternateFlow) {
        ...
        this.getTokenslock = new ReentrantLock();
    }
    ...
    public AuthResponse getTokens(Duration minValidity) throws IOException {
        AuthResponse currentResponse = this.cachedResponse;
        if (currentResponse == null
                || currentResponse.tokens.getAccessToken() == null
                || currentResponse.expiration.isBefore(Instant.now().plus(minValidity))) {
    ...
    private AuthResponse refreshTokens() throws IOException {
        ...
        try {
            ...
            if (!this.getTokenslock.tryLock(10, TimeUnit.SECONDS))
