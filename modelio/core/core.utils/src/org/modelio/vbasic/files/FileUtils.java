...
public final class FileUtils {
    ...
    @objid ("b502e215-fd52-4bda-b8df-66320d2e895e")
    ...
    @objid ("4fce9074-7660-4b58-95c6-5a6b06db95a0")
    private static void forceDeleteFile(Path file) throws IOException {
        try {
            Files.delete(file);
            ...
            // Permissions attributes are OS and file system dependent
            try {
                FileStore fileStore = Files.getFileStore(file);
                if (fileStore.supportsFileAttributeView(DosFileAttributeView.class)) {
                    // try DOS read only attribute in all cases
                    try {
                        Files.setAttribute(file, "dos:readonly", false, LinkOption.NOFOLLOW_LINKS);
                    } catch (IOException | SecurityException ex) {
                        // ignore, we will try POSIX or ACL permissions
                        e.addSuppressed(ex);
                    }
                }

                // Try POSIX or ACL permissions
                try {
                    if (fileStore.supportsFileAttributeView("posix")) {
                        Files.setAttribute(file, "posix:permissions", java.util.EnumSet.of(
                                java.nio.file.attribute.PosixFilePermission.OWNER_WRITE,
                                java.nio.file.attribute.PosixFilePermission.OWNER_READ), LinkOption.NOFOLLOW_LINKS);
                    } else if (fileStore.supportsFileAttributeView(AclFileAttributeView.class)) {
                        // Probably Windows NTFS
                        AclFileAttributeView view = Files.getFileAttributeView(file, AclFileAttributeView.class, LinkOption.NOFOLLOW_LINKS);
                        var acls = view.getAcl();
                        @SuppressWarnings ("resource")
                        var user =  file.getFileSystem().getUserPrincipalLookupService().lookupPrincipalByName(System.getProperty("user.name"));
                    ...
                    // ignore, we tried our best effort
                    e.addSuppressed(ex);
                }
                ...
                // Add to the original exception as suppressed and rethrow the original.
                e.addSuppressed(ex);
            }
            throw e;
