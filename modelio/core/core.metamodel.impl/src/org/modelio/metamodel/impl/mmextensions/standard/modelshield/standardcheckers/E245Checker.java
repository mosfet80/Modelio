 ...
 * <li>what = The ''{0}'' part can belong to only one of the following elements : ''{1}''.</li>
 * </ul>
 */
...
public class E245Checker implements IChecker {
    ...
    public void check(MObject object, final IErrorReport report) {
        ...
        if (cluster != null) {
            parents.add(cluster.getName());
        }

        Classifier internalOwner = currentBindableInstance.getInternalOwner();
        if (internalOwner != null) {
            parents.add(internalOwner.getName());
        }

        NameSpace owner = currentBindableInstance.getOwner();
        if (owner != null) {
            parents.add(owner.getName());
        }

        if (parents.size() > 1) {
            report.addEntry(new ModelError(ERRORID, object, Arrays.asList(String.join(", ", parents))));
        }
