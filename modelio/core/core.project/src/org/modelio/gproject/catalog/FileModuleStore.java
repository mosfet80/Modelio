 ...
 * Furthermore, as finding all the modules remains a costly operation (looping in all the store module directories), an internal cache is managed as a Map of FileModuleStoreEntry.
 * <p>
...
public class FileModuleStore implements IModuleStore {
    ...
    /**
     * The directory where this FileModuleStore will persist its contents.
     */
    @objid ("28693d33-ad95-4306-8cf7-5f5811820ed5")
    ...
    protected void migration() {
        ...
        if (isMigrationNeeded) {
            try {
                ...
                Log.trace("'" + this.storePath + "' module catalog cleaning done");
            } catch (FileSystemException e) {
                Log.warning("Cannot migrate '" + this.storePath + "' module catalog from " + version + " to " + getRequiredConfVersion());
    ...
    protected static final class FileModuleStoreEntry {
         ...
         */
        @objid ("beb81e14-11b4-4ebe-80c7-d59a9ecb280c")
        private final Path entryPath;

        /**
         * The path of the module expanded data file (a directory)
         */
        @objid ("1981a4c7-19f7-41db-8499-067f506a7e28")
        ...
        private FileModuleStoreEntry(Path entryPath, Collection<IGMetamodelExtension> metamodelFragments, IModuleHandle refHandle) throws IOException {
            ...
            this.moduleHandle = new FileModuleStoreHandle(datapath,
                    ...
                    refHandle.getBinaryVersion(), refHandle.getDependencies(),
    ...
    /**
     * Cache states: - INITIAL the cache is empty and has not been loaded yet. - VALID the cache has been loaded and further operations (adding/removing) could update it in order to keep its contents valid.
     */
    @objid ("d5396a79-db7d-4e82-80f4-245c1314007f")
    protected enum FileModuleStoreState {
