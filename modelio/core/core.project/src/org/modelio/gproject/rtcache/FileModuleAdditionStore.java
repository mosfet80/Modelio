...
/**
 * Extension of {@link FileModuleStore} systematically creating new directories when a module is installed in the cache,
 * to avoid problems with locked jar files...
 * <p>
 * They are stored in the cache as a directory $(jmdac name)_$(jmdac version)/X, where X is the number of modules having the
 * same version already installed in the cache.
 * </p>
 */
...
class FileModuleAdditionStore extends FileModuleStore {
    ...
    protected void load(IModelioProgress monitor) {
        this.entries.clear();
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(this.storePath, file -> Files.isDirectory(file))) {
            for (Path p : stream) {
