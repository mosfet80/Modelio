...
public class UndoSubProcessTransformer implements IModelTransformer {
    ...
    private IDiagramNode getDiagramGraphic(IDiagramHandle dh, MObject e) {
        ...
        if (!dgs.isEmpty()) {
        IDiagramGraphic dg = dgs.get(0);
        if (dg instanceof IDiagramNode)
        return (IDiagramNode) dg;
        }

        return null;
    }

    @objid ("68876145-f74a-4095-b68a-cef115fc66b1")
    private IDiagramLink getDiagramLink(IDiagramHandle dh, MObject e) {
        List<IDiagramGraphic> dls = dh.getDiagramGraphics(e);
        if (!dls.isEmpty()) {
        IDiagramGraphic dl = dls.get(0);
        if (dl instanceof IDiagramLink)
        return (IDiagramLink) dl;
        }
    ...
    public List<MObject> transform(AbstractDiagram diagram, ISelection selection) {
        ...
        AbstractDiagram subProcessDiagram = subProcess.getProduct().get(0);
        Map<MObject, Rectangle> unmaskCoordinates = new HashMap<>();
        Map<MObject, ILinkRoute> linkGeometries = new HashMap<>();
        ...
        try (IDiagramHandle dh = diagramService.getDiagramHandle(diagram)) {

        IDiagramHandle dhSubProcess = modelioServices.getDiagramService().getDiagramHandle(subProcessDiagram);

        dh.setBatchMode(true);

        // open the subprocess diagram
        IDiagramNode diagramGraphic = getDiagramGraphic(dh, subProcess);
        Rectangle spBounds = new Rectangle();
        if (diagramGraphic != null)
        spBounds = diagramGraphic.getBounds().getCopy();
        Point offset = spBounds.getTopLeft();
        ...
        List<BpmnDataAssociation> dataLinks = new ArrayList<>();
        ...
        } else if (n instanceof BpmnItemAwareElement) {
        nodes.add(n);
        BpmnItemAwareElement temp = (BpmnItemAwareElement) n;
        for (BpmnDataAssociation dataLink : temp.getSourceOfDataAssociation()) {
        dataLinks.add(dataLink);
        }
        for (BpmnDataAssociation dataLink : temp.getTargetOfDataAssociation()) {
        dataLinks.add(dataLink);
        }
        } else {
        nodes.add(n);
        ...
        IDiagramNode inputDg = getDiagramGraphic(dhSubProcess, inputNode);
        setFlowElementParent(inputNode, parent);

        if (inputDg != null) {
        unmaskCoordinates.put(inputNode, inputDg.getBounds().getCopy().getTranslated(offset));
        inputDg.mask();
        inputDg = null;
        } else {
        ...
        IDiagramNode outputDg = getDiagramGraphic(dhSubProcess, outputNode);
        setFlowElementParent(outputNode, parent);

        if (outputDg != null) {
        unmaskCoordinates.put(outputNode, outputDg.getBounds().getCopy().getTranslated(offset));
        outputDg.mask();
        outputDg = null;
        ...
        IDiagramNode nodeDg = getDiagramGraphic(dhSubProcess, n);

        if (nodeDg != null) {
        unmaskCoordinates.put(n, nodeDg.getBounds().getCopy().getTranslated(offset));
        nodeDg.mask();
        nodeDg = null;
        } else {
        ...
        IDiagramLink linkDg = getDiagramLink(dh, flow);
        if (linkDg != null) {
        linkDg.mask();
        dh.unmask(flow, 0, 0);
        }
        ...
        coordinates = coordinates.getTranslated(offset);
        ...
        for (BpmnSequenceFlow link : links) {
        layoutLink(dh, linkGeometries, offset, link);
        }

        for (BpmnDataAssociation link : dataLinks) {
        layoutLink(dh, linkGeometries, offset, link);
        }
    ...
    @objid ("031c65ad-7be3-45e6-b40d-5cc47efd1a1f")
    ...
    public boolean canExecute(AbstractDiagram diagram, ISelection selection) {
        ...
        if (subProcess.getFlowElement(BpmnStartEvent.class).size() != 1
        || subProcess.getFlowElement(BpmnEndEvent.class).size() != 1
        || subProcess.getProduct(BpmnSubProcessDiagram.class).isEmpty()) {
        return false;
        }
        }
        return true;
