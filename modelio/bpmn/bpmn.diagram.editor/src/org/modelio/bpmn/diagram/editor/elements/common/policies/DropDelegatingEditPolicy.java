 ...
 */
@objid ("6118fcf7-55b6-11e2-877f-002564c97630")
public class DropDelegatingEditPolicy extends DefaultElementDropEditPolicy {
    @objid ("6118fcfb-55b6-11e2-877f-002564c97630")
    @Override
    protected EditPart getDropTargetEditPart(ModelElementDropRequest request) {
        ...
        if (delegate) {
            ...
            GmCompositeNode gmLaneSetContainer = ((GmCompositeNode) getHost().getModel()).getCompositeFor(BpmnLane.class);
            if (gmLaneSetContainer != null) {
                // We have the model, lets find the corresponding edit part
                EditPart partitionContainerEditPart = (EditPart) getHost().getViewer()
                        .getEditPartRegistry()
                        .get(gmLaneSetContainer);
