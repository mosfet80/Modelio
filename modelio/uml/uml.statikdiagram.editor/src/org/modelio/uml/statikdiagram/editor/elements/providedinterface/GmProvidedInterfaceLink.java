...
public class GmProvidedInterfaceLink extends GmLink {
    ...
    public MObject getFromElement() {
        ProvidedInterface relatedElement = this.getRelatedElement();
        if (relatedElement == null)
            return null;
        else
            return relatedElement.getProviding();
    ...
    public Image getImage() {
        ProvidedInterface relatedElement = getRelatedElement();
        if (relatedElement == null) {
            return MetamodelImageService.getImage(getRelatedMClass());
        } else {
            return ElementImageService.getImage(relatedElement);
        }
    ...
    public void readLink(IDiagramReader in) {
        super.readLink(in);
        this.element = (ProvidedInterface) resolveRef(getRepresentedRef());
    ...
    public Collection<NaryConnector> getConnections() {
        ...
        final Collection<NaryConnector> ret = new ArrayList<>(5);
        for (NaryLinkEnd l : relatedElement.getNaryConsumer(NaryConnectorEnd.class)) {
