...
public class ActivityDiagramEditorInputProvider implements IDiagramEditorInputProvider {
    ...
    public DiagramEditorInput compute(String diagramUID, IModelManager modelManager) {
        ActivityDiagram diagram = (ActivityDiagram) modelManager.getModelingSession().getModel().findById(ActivityDiagram.class, diagramUID);
        return diagram != null ? new ActivityDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }

    @objid ("a3889ef5-c668-4535-8a84-d2ecb379ebfe")
    @Override
    public DiagramEditorInput compute(AbstractDiagram diagram, IModelManager modelManager) {
        return diagram instanceof ActivityDiagram ? new ActivityDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }
