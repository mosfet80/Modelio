...
public class SequenceDiagramEditorInputProvider implements IDiagramEditorInputProvider {
    ...
    public DiagramEditorInput compute(String diagramUID, IModelManager modelManager) {
        SequenceDiagram diagram = (SequenceDiagram) modelManager.getModelingSession().getModel().findByRef(new MRef(SequenceDiagram.MQNAME, diagramUID));
        return diagram != null ? new SequenceDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }

    @objid ("71e155cb-2e2a-443c-8d05-ba1a7d147d02")
    @Override
    public DiagramEditorInput compute(AbstractDiagram diagram, IModelManager modelManager) {
        return diagram instanceof SequenceDiagram? new SequenceDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }
