...
public class ObjectDiagramEditorInputProvider implements IDiagramEditorInputProvider {
    ...
    public DiagramEditorInput compute(String diagramUID, IModelManager modelManager) {
        AbstractDiagram diagram = (AbstractDiagram) modelManager.getModelingSession().getModel().findByRef(new MRef(ObjectDiagram.MQNAME, diagramUID));
        return diagram != null ? new ObjectDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }

    @objid ("b354396e-eb64-4254-bcc8-4de30b732a3f")
    @Override
    public DiagramEditorInput compute(AbstractDiagram diagram, IModelManager modelManager) {
        return diagram instanceof ObjectDiagram ? new ObjectDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }
