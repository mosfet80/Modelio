...
public class UseCaseDiagramEditorInputProvider implements IDiagramEditorInputProvider {
    ...
    public DiagramEditorInput compute(String diagramUID, IModelManager modelManager) {
        UseCaseDiagram diagram = (UseCaseDiagram) modelManager.getModelingSession().getModel().findByRef(new MRef(UseCaseDiagram.MQNAME, diagramUID));
        return diagram != null ? new UseCaseDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }

    @objid ("072cd047-fafb-4218-9b32-16e582812db2")
    @Override
    public DiagramEditorInput compute(AbstractDiagram diagram, IModelManager modelManager) {
        return diagram instanceof UseCaseDiagram ? new UseCaseDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }
