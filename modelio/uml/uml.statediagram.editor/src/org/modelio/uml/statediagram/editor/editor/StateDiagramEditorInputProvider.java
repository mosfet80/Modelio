...
public class StateDiagramEditorInputProvider implements IDiagramEditorInputProvider {
    ...
    public DiagramEditorInput compute(String diagramUID, IModelManager modelManager) {
        StateMachineDiagram diagram = (StateMachineDiagram) modelManager.getModelingSession().getModel().findByRef(new MRef(StateMachineDiagram.MQNAME, diagramUID));
        return diagram != null ? new StateDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }

    @objid ("b452df0e-c8ad-4162-9324-3771509a5bf6")
    @Override
    public DiagramEditorInput compute(AbstractDiagram diagram, IModelManager modelManager) {
        return diagram instanceof StateMachineDiagram ? new StateDiagramEditorInput(modelManager, diagram, getDiagramCreator()) : null;
    }
