...
public class StateGmLinkFactory implements IGmLinkFactory {
    ...
    public Class<? extends IPersistentMigrator> resolveMigratorClass(String classNamespace) {
        try { String fixedNamespace = migrateNamespacing(classNamespace);
            Class<?> clazz = Class.forName(fixedNamespace);
            if (clazz != null) {
    ...
    public Class<? extends Enum<?>> resolveEnumClass(String enumNamespace) {
        try { String fixedNamespace = migrateNamespacing(enumNamespace);
            Class<?> clazz = Class.forName(fixedNamespace);
    ...
    @objid ("853b12df-ddde-4cea-b991-a4ccf24b5988")
    @Override
