...
**If exists:**
Use the `ask_questions` tool (`functions.ask_questions`):
...
**Then use the `ask_questions` tool (`functions.ask_questions`) with `multiSelect: true`:**
...
2. **Ask 4 questions using the `ask_questions` tool (`functions.ask_questions`):**
   - header: "[Area]"
   - question: Specific decision for this area
   - options: 2-3 concrete choices (the `ask_questions` tool might add "Other" automatically when allowed)
