...
tools: ["read/readFile", "edit/editFiles", "execute/runInTerminal", "agent", "vscode/askQuestions"]
---
...
- Use the `ask_questions` tool to gather feature ideas through conversation
...
Store resolved models for use in agent spawning below.

## Phase 7: Research Decision

Use the `ask_questions` tool:
  ...
  questions: [
    {
      ...
      options: [
        { label: "Research first (Recommended)", description: "Discover patterns, expected features, and architecture for NEW capabilities", recommended: true },
        { label: "Skip research", description: "I know what I need, go straight to requirements" }
      ]
    }
  ]
})
...
Display spawning indicator and execute parallel runSubagent calls:
...
For each category, use the `ask_questions` tool:
  ...
  questions: [
    {
      ...
      options: [
        ...
        { label: "None for this milestone", description: "Defer entire category" }
      ]
    }
  ]
})
  ...
  questions: [
    {
      ...
      options: [
        ...
        { label: "Yes, let me add some", description: "Capture additional requirements" }
      ]
    }
  ]
})
...
Use the `ask_questions` tool for confirmation:
...
ask_questions({
  questions: [
    {
      header: "Requirements",
      question: "Does this capture what you're building for this milestone?",
      options: [
        { label: "Yes", description: "Proceed to roadmap" },
        { label: "Adjust", description: "Return to scoping" }
      ]
    }
  ]
})
