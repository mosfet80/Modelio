...
Before spawning, read file contents. The `@` syntax does not work reliably across agent boundaries.
...
Spawn all plans in a wave with parallel runSubagent calls:

```
runSubagent(
  agentName: "⚡ GSD Executor",
  description: "Execute {plan_01_id}",
  model: "{executor_model}",
  prompt: "Execute plan at {plan_01_path}\n\nPlan:\n{plan_01_content}\n\nProject state:\n{state_content}"
)
runSubagent(
  agentName: "⚡ GSD Executor",
  description: "Execute {plan_02_id}",
  model: "{executor_model}",
  prompt: "Execute plan at {plan_02_path}\n\nPlan:\n{plan_02_content}\n\nProject state:\n{state_content}"
)
runSubagent(
  agentName: "⚡ GSD Executor",
  description: "Execute {plan_03_id}",
  model: "{executor_model}",
  prompt: "Execute plan at {plan_03_path}\n\nPlan:\n{plan_03_content}\n\nProject state:\n{state_content}"
)
```

All three run in parallel via runSubagent parallelization.

**No polling.** No background agents. No blocking output streams.
</wave_execution>
