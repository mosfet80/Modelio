...
public class SatelliteLabelMoveResizeEditPolicy extends DefaultNodeResizableEditPolicy {
    ...
    private Dimension computeBestSize(ChangeBoundsRequest request, Rectangle askedBounds) {
        ...
        // Always honour the user's requested size in all directions.
        // getPreferredSize() returns the minimum text size which would prevent
        // the user from making the label larger than its text content.
        Dimension prefered = new PrecisionDimension(askedBounds.width(), askedBounds.height());

        // If only resizing vertically, recompute height to fit content at the asked width
        Dimension delta = request.getSizeDelta();
        if (delta.width() == 0) {
            // Pure vertical resize: recompute height to fit content
            prefered.height = fig.getPreferredSize(askedBounds.width(), -1).height;
        }
