...
public class DiagramCreationHandler {
    @objid ("a2138d34-8a4b-4202-b3e9-e1414d710c50")
    @Inject
    ...
    @Named("contributor") final String contributorName) {
        ...
        // Trivial cases
        if (selectedElement == null || selectedElement.getStatusLazy().isRamc()) {
            return false;
        }

        // Search for the first compatible contributor which accepts the
        // selected element
        for (List<IWizardContributor> contributors : getPossibleContributors().values()) {
            for (IWizardContributor contributor : contributors) {
    ...
    @Named("contributor") final String contributorName, @Named(IServiceConstants.ACTIVE_SHELL) final Shell activeShell) {
        ...
        if (this.dialog == null || this.dialog.getShell() == null) {
            Map<ContributorCategory, List<IWizardContributor>> contributors = getPossibleContributors();

            // Create an Open dialog
            CreationWizardModel resultModel = new CreationWizardModel(contributors);
            resultModel.setContext(selectedElement);
            resultModel.setTypeFilter(IDiagramWizardContributor.class);

            IWizardContributor preselectedContributor = findContributorById(contributorName, contributors.values());

            if (preselectedContributor == null) {
                // No preselected contributor => prompt user
                preselectedContributor = promptUser(activeShell, resultModel);
            } else {
                // The command being executed has provided a contributor, no
                // user prompt
                resultModel.setName(preselectedContributor.getLabel());
                resultModel.setSelectedContributor(preselectedContributor);
            }
            ...
            if (preselectedContributor != null) {
                ...
                try (ITransaction t = session.getTransactionSupport().createTransaction("Create element")) {
                    MObject elt = preselectedContributor.actionPerformed(resultModel.getContext(), resultModel.getName(),
    ...
    private IWizardContributor findContributorById(String contributorName, Collection<List<IWizardContributor>> contributors) {
        if (contributorName == null) {
            return null;
        }

        for (List<IWizardContributor> contributorList : contributors) {
            for (IWizardContributor contributor : contributorList) {
    ...
    private Map<ContributorCategory, List<IWizardContributor>> getPossibleContributors() {
        Map<ContributorCategory, List<IWizardContributor>> ret = new HashMap<>();

        CreationContributorManager manager = Objects.requireNonNull(this.context.get(CreationContributorManager.class));

        for (ContributorCategory category : manager.getCategories()) {
            List<IWizardContributor> selectedContributors = new ArrayList<>();

            for (IWizardContributor contributor : manager.getContributions(category)) {
                ElementDescriptor createdElementType = contributor.getCreatedElementType();
                if (IDiagramWizardContributor.class.isAssignableFrom(contributor.getClass())
                        && createdElementType != null
                        && createdElementType.getMClass() != null
                        && createdElementType.getMClass().isEnabled()) {
                    selectedContributors.add(contributor);
                }
            }

            if (!selectedContributors.isEmpty()) {
                ret.put(category, selectedContributors);
            }
        }
        return ret;
    }
    ...
    private static ModelElement getSelectedElement(final Object selection) {
        return InputHelper.getFirst(selection, ModelElement.class);
    }
