...
@objid ("65afb5c4-33f7-11e2-95fe-001ec947c8cc")
public class BottomAlignHandler extends AbstractAlignHandler {
    @objid ("65afb5c5-33f7-11e2-95fe-001ec947c8cc")
    @Override
    protected void align(GraphicalEditPart primarySelection, List<GraphicalEditPart> otherSelections) {
        ...
        CompoundCommand compound = new CompoundCommand("Align bottom");
        for (GraphicalEditPart editPart : otherSelections) {
            IFigure figure = editPart.getFigure();
            Rectangle bounds = getEffectiveBounds(figure);
            figure.translateToAbsolute(bounds);

            Point moveDelta = new Point(0, primaryBounds.y + primaryBounds.height - (bounds.y + bounds.height));
