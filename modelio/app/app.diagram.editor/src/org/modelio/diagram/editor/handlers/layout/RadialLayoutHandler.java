 ...
 * </p>
 */
@objid ("3a0be098-2a84-457a-a3f0-1eccb2bd70e5")
public class RadialLayoutHandler {
    @objid ("5d496371-194a-4cb2-9780-f164b1e6a88d")
    @Execute
    public void execute(@Named(IServiceConstants.ACTIVE_SELECTION) final ISelection selection, @Named(IServiceConstants.ACTIVE_PART) final MPart part, IProjectService projectService, IModelioEventService eventService, IEclipseContext eclipseContext) {
        ...
        try (ITransaction transaction = projectService.getSession().getTransactionSupport()
                .createTransaction("RadialLayoutHandler - > Run Radial Layout");) {

            DiagramLayoutEngine.createLayoutAlgorithme(LayoutAlgorithmeType.RADIAL).withDiagram(diagram, diagramService)
    ...
    @objid ("de257a8e-59bc-4183-9e0e-25f030189905")
    @CanExecute
