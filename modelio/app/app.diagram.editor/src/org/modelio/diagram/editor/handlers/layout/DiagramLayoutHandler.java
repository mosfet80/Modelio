...
/**
 * Handler for Diagrams layouts.
 * <p>
 * This handler is used to apply a tree layout to the selected elements in the
 * diagram editor. It uses the {@link DiagramLayoutEngine} to create and run
 * the layout algorithm.
 * </p>
 */
@objid ("840ee320-2271-4cc8-9021-82e2bc526fc8")
public class DiagramLayoutHandler {
    @objid ("6c6dc01c-e6b3-44b7-ab90-7e9eee1415fe")
    @Execute
    public void execute(@Named(IServiceConstants.ACTIVE_SELECTION) final ISelection selection, @Named(IServiceConstants.ACTIVE_PART) final MPart part, IProjectService projectService, IModelioEventService eventService, IEclipseContext eclipseContext) {
        ...
        try (ITransaction transaction = projectService.getSession().getTransactionSupport() .createTransaction("Run Automatic Diagram layout");) {
            ILayoutAlgorithme layoutAlgorithme = DiagramLayoutEngine.createLayoutAlgorithme(diagram, diagramService);
            layoutAlgorithme.runLayout();
            layoutAlgorithme.compactDiagramBounds(50);
            layoutAlgorithme.postProcessing();
            transaction.commit();
        } catch (Exception e) {
