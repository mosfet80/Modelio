...
@objid ("65c52b04-33f7-11e2-95fe-001ec947c8cc")
public class RightAlignHandler extends AbstractAlignHandler {
    @objid ("65c52b05-33f7-11e2-95fe-001ec947c8cc")
    @Override
    protected void align(GraphicalEditPart primarySelection, List<GraphicalEditPart> otherSelections) {
        ...
        CompoundCommand compound = new CompoundCommand("Align right");
        for (GraphicalEditPart editPart : otherSelections) {
            IFigure figure = editPart.getFigure();
            Rectangle bounds = getEffectiveBounds(figure);
            figure.translateToAbsolute(bounds);

            Point moveDelta = new Point(primaryBounds.x + primaryBounds.width - (bounds.x + bounds.width), 0);
