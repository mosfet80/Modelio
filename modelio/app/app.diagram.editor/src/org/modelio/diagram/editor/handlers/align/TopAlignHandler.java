...
@objid ("65c78d9a-33f7-11e2-95fe-001ec947c8cc")
public class TopAlignHandler extends AbstractAlignHandler {
    @objid ("65c78d9b-33f7-11e2-95fe-001ec947c8cc")
    @Override
    protected void align(GraphicalEditPart primarySelection, List<GraphicalEditPart> otherSelections) {
        ...
        CompoundCommand compound = new CompoundCommand("Align top");
        for (GraphicalEditPart editPart : otherSelections) {
            IFigure figure = editPart.getFigure();
            Rectangle bounds = getEffectiveBounds(figure);
            figure.translateToAbsolute(bounds);

            Point moveDelta = new Point(0, primaryBounds.y - bounds.y);
