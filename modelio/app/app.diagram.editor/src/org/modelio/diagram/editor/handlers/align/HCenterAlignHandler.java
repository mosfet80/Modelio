...
@objid ("65afb61e-33f7-11e2-95fe-001ec947c8cc")
public class HCenterAlignHandler extends AbstractAlignHandler {
    @objid ("65afb61f-33f7-11e2-95fe-001ec947c8cc")
    @Override
    protected void align(GraphicalEditPart primarySelection, List<GraphicalEditPart> otherSelections) {
        ...
        CompoundCommand compound = new CompoundCommand("Align horizontal center");
        for (GraphicalEditPart editPart : otherSelections) {
            IFigure figure = editPart.getFigure();
            Rectangle bounds = getEffectiveBounds(figure);
            figure.translateToAbsolute(bounds);

            Point moveDelta = new Point(primaryBounds.x + primaryBounds.width / 2 - (bounds.x + bounds.width / 2), 0);
