        ...
        IRunnableWithProgress runnable = monitor -> {
            ...
            for (IGModelFragment f : frags) {
                if (monitor.isCanceled()) {
                    globRes.add(Status.CANCEL_STATUS);
                    // Exit the for loop
                    break;
                }

                try {
                    BrowserViewActivator.LOG.info("Trying to open '%s' %s model fragment...", f.getId(), f.getType());
                    ...
                    BrowserViewActivator.LOG.info("  Opened '%s' %s model fragment.", f.getId(), f.getType());
                } catch (GPartException e) {
                    BrowserViewActivator.LOG.warning("  FAILED opening '%s' %s model fragment: %s.", f.getId(), f.getType(), e.getLocalizedMessage());
