...
We are pleased to announce the release of Modelio 6.2 

link:https://github.com/ModelioOpenSource/Modelio/releases/tag/v6.2.0[*Modelio 6.2 Download*]

https://github.com/ModelioOpenSource/Modelio/wiki[*Modelio 6.2 Documentation*]

=== Release Note
https://github.com/ModelioOpenSource/Modelio/wiki/Modelio-Release-6.2[*Modelio 6.2 Release Note*]

Modelio 6.2 introduces a series of major improvements focused on usability and model exploration. The flagship addition is a brand-new visual query tool that allows all users,technical or not, to build powerful, targeted searches across their models without writing any code. Queries can be filtered by element type, name, properties, and relationships, making it significantly easier to navigate large and complex architectures. This release also brings automatic diagram layout, which intelligently reorganizes graphical elements using algorithms such as tree layout and overlap removal, and is accessible both interactively and through the module API for automation scenarios.

On the modeling side, ArchiMate support has been substantially strengthened: elements can now be transformed from one type to another while preserving their diagram positions, graphical representations have been updated for better standard compliance, and duplicate links are now correctly displayed on import. The links editor and diagram tooling have also received targeted refinements, including case-insensitive element search, link name display in unmask dialogs, and a more visible graphical properties panel. The bug fix stream across the 6.0, 6.1, and 6.2 patch releases addresses stability in Teamwork collaboration, BPMN lane management, HTML notes editing, XMI import/export, and general diagram rendering.
